...
export const suggestBrainkey = dictionary => key.suggest_brain_key(dictionary);

export const suggestPassword = () => `P${key.get_random_key().toWif().substr(0, 45)}`;
...
const encodeBody = params => Object.keys(params).map(bodyKey => `${encodeURIComponent(bodyKey)}=${encodeURIComponent(params[bodyKey])}`).join('&');

export const createAccount = async ({
  name, activeKey, ownerKey, email,
}) => {
